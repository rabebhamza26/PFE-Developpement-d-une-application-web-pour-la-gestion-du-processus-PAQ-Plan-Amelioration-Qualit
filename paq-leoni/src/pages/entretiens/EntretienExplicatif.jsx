 ...
 const loadEmails = async () => {
  try {
    ...
    if (response.ok) {
      ...
      console.log("Emails chargÃ©s:", emails);
  ...
  const addTypeOption = async () => {
    ...
    try {
      ...
      setStatusMessage("Type de dÃ©faut ajoutÃ© avec succÃ¨s.");
      showSuccessToast("Faute ajoutée");
    } catch (err) {
      setError("Erreur lors de l'ajout du dÃ©faut.");
  ...
  const handleAjouter = () => {
    resetForm();
    setStatusMessage("Nouveau formulaire prÃªt Ã  Ãªtre rempli.");
    setError("");
  ...
  const handleModifier = async () => {
    if (entretiensList.length === 0) {
      setError("Aucun entretien existant Ã  modifier.");
      return;
    }
  ...
  const handleDeleteConfirm = async (destinataireEmail, message) => {
    ...
    try {
      ...
      resetForm();
      await loadAllEntretiens(); // Recharger la liste aprÃ¨s suppression
      setStatusMessage("Entretien supprimÃ© avec succÃ¨s. Email envoyÃ©.");
      await showSuccessAlert("Entretien supprimé", "L'entretien explicatif a bien été supprimé.");
  ...
  const handleSubmitConfirm = async (destinataireEmail, message) => {
    ...
    try {
      ...
      if (currentEntretienId) {
        ...
        setStatusMessage("Entretien modifiÃ© avec succÃ¨s. Email envoyÃ©.");
        ...
        setStatusMessage("Entretien crÃ©Ã© avec succÃ¨s. Email envoyÃ©.");
        await showSuccessAlert("Entretien créé", "L'entretien explicatif a été créé avec succès.");
      }

      localStorage.removeItem(`entretien-explicatif-draft-${matricule}`);
      await loadAllEntretiens(); // Recharger la liste aprÃ¨s crÃ©ation/modification
      setTimeout(() => navigate(`/paq-dossier/${matricule}`), 1500);
    } catch (err) {
                      ...
                      {showDropdown && (
                        <div className="leoni-dropdown">
                          {filteredFautes.length > 0 ? (
                            ...
                            <div className="leoni-dropdown-empty">Aucun rÃ©sultat</div>
                          )}
                        </div>
                      )}
                    </div>
